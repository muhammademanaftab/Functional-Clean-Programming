...
(--) (x1,y1,z1) (x2,y2,z2) = ((x2 - x1)^2.0 + (y2 - y1)^2.0 + (z2 - z1)^2.0)^(0.5) 
...
str_to_chr :: String -> [Char]
str_to_chr str = [a \\ a<-: str]

remove_vowel :: [Char] -> [Char]
remove_vowel lst = [a \\ a<- lst | not (isMember a vowelList)]

arr_to_lst :: {String} -> [[Char]]
arr_to_lst arr = [str_to_chr a \\ a <-: arr ] 

final_remove :: [[Char]] -> [[Char]]
final_remove chr = [ remove_vowel a \\ a<- chr]

back_1:: [[Char]] -> [String]
back_1 chr = [toString a \\ a <- chr]

lst_to_arr :: [[String]] -> {[String]}
lst_to_arr lst =  {a \\ a<- lst}

basic_arr :: [String] -> {String}
basic_arr ls = {a \\ a<- ls}

lst_arr_help2 :: {[String]} -> {{String}}
lst_arr_help2 ls = {basic_arr a \\ a<-:ls}


remove_end :: {{String}} -> {{String}}
remove_end arr = lst_arr_help2 (lst_to_arr [(back_1 a) \\ a<- chr])
	where chr = [(final_remove (arr_to_lst a)) \\ a<-:arr]




remove_end1 :: {{String}} -> {{String}}
remove_end1 a_o_a_o_f_string = {  { {c \\ c<-:str | not (isMember c vowelList)} \\ str<-:arr_of_str} \\ arr_of_str<-:a_o_a_o_f_string}

//Start = remove_vowels ['h','e','i','l']
//Start = remove_end{{"hello","world"},{"good","morning"}} 
// {{"hll","wrld"},{"gd","mrnng"}}
//Start = remove_end {{"HELLO","WORLD"},{"GOOD","EVENING"},{"hello","again!"}} 
// {{"HLL","WRLD"},{"GD","VNNG"},{"hll","gn!"}}
//Start = remove_end {{"","o"},{"aA","oOo","AEIOUaeiou"}} // {{"",""},{"","",""}}
...
//Start =(isMember IK Line2.stops )

FindLinesC :: Stop Transport -> Bool
FindLinesC stp trns 
|  ((isMember stp Line1.stops) || (isMember stp Line7.stops) || (isMember stp Line4.stops)) &&(trns== Bus) = True
|  ((isMember stp Line6.stops) || (isMember stp Line3.stops))&&(trns== Metro) = True
|  ((isMember stp Line2.stops) || (isMember stp Line5.stops))  && (trns== Tram) = True
=False
...
FindLinesR stp = [(x.number,x.transport) \\ x<- Linelist | (isMember stp x.stops)]
...
FindLinesB stp_1 stp_2 = [(x.transport,x.number) \\ x<-Linelist | ((isMember stp_1 x.stops) && (isMember stp_2 x.stops))]
...
FindLinesT stp_1 stp_2 trans = [(x.number) \\ x <- Linelist | ((isMember stp_1 x.stops) && (isMember stp_2 x.stops) && (trans==x.transport))]
...
//helper :: (TreeT Int) -> Bool
//helper Leaf = True
//helper (Node x le ri) 
//|Node x == aver_sub x && helper le && helper ri
//=False
