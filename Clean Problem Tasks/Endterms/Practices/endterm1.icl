...
| length onlyBSc == 0 = "Error"
= hd [ s.id \\ s<-onlyBSc & a<-avgBSc | a == maxEl ]
	where onlyBSc = [ s \\ s<-list | s.level == BSc ]
		  avgBSc = [ avg s.grades \\ s<-onlyBSc ]
		  maxEl = maxList avgBSc
...
occ ::  Char String -> Int
occ c str = length [ 1 \\ s<-:str | s == c ]

count :: String -> {(Char, Int)}
count str = { (c,(occ c str)) \\ c<-unique_chars | occ c str >= 2 }
	where unique_chars = removeDup [ c \\ c<-:str ]
...
//-
instance - [Int]
	where (-) a b = [x \\ x<-a | not (isMember x b)]
...
getBTHeight :: BinaryTree Int -> Int
getBTHeight BTLeaf _ = 1
getBTHeight (BTNode le ri) accum = 1 + (max (getBTHeight le (accum+1)) (getBTHeight ri(accum+1)))
//Start = getBTHeight BTLeaf 0 // 1
//Start = getBTHeight bt1 0 // 3
//Start = getBTHeight bt2 0 // 3
//Start = getBTHeight bt3 // 5
//Start = getBTHeight bt4 0 // 7
...
LinkedListToList :: (LinkedList String) -> [String]
LinkedListToList Nil = []
LinkedListToList (Pointer s next) = [s] ++ LinkedListToList next

ListToLinkedList :: [String] -> (LinkedList String)
ListToLinkedList [] = Nil
ListToLinkedList [x:xs] = Pointer x (ListToLinkedList xs)

Reverse :: (LinkedList String) -> (LinkedList String)
Reverse t  = ListToLinkedList list
	where list = reverse (LinkedListToList t)
	
getLast :: (LinkedList String) -> String
getLast (Pointer s Nil) = s
getLast (Pointer s next) = getLast next

removeLast :: (LinkedList String) -> (LinkedList String)
removeLast (Pointer s Nil) = Nil
removeLast (Pointer s next) = Pointer s (removeLast next)

Reverse2 :: (LinkedList String) -> (LinkedList String)
Reverse2 Nil = Nil
Reverse2 p = Pointer (getLast p ) (Reverse  (removeLast p) )

//Start = Reverse2 linkedlist1 //(Pointer "H" (Pointer "e" (Pointer "l" (Pointer "l" (Pointer "o" Nil)))))
//Start = Reverse2 (Insert linkedlist1 "World") //(Pointer "World" (Pointer "H" (Pointer "e" (Pointer "l" (Pointer "l" (Pointer "o" Nil))))))
//Start = Reverse linkedlist2 // (Pointer "H" (Pointer "e" (Pointer "y" Nil)))
//Start = Reverse2 Nil // Nil
...
delete Nil str = Nil
delete (Pointer s next) str
|s==str = delete next str
= Pointer s (delete next str)
...
concat Nil l2 = l2
concat l1 Nil = l1
concat (Pointer s Nil) p = Pointer s p 
concat (Pointer s next) p = Pointer s (concat next p)
...
instance == BasicPersonAccount
	where (==) a b = (a.name == b.name) && (a.ip == b.ip) && (a.age == b.age)

dequeFakes :: BasicPersonAccount [BasicPersonAccount] -> [BasicPersonAccount]
dequeFakes og [] = [] 
dequeFakes og [x:xs]
| (x.name == og.name) && (x.ip == og.ip) && (x.age <> og.age) = [{ x & inQueue = False }: dequeFakes og xs]
= [x : dequeFakes og xs]

aux :: [BasicPersonAccount] -> [BasicPersonAccount]
aux [x] = [x]
aux [x:xs]
| isMember x xs = [x: dequeFakes x xs]
= [x: aux xs]

findFakes :: {BasicPersonAccount} -> {BasicPersonAccount}
findFakes arr
| length list == 0 = {}
| otherwise = { r \\ r<- removeDup(aux list)}
	where list = [b \\ b<-:arr]
...
largestMat :: {[[Int]]} -> [[Int]]
largestMat arr_mats = hd [ m \\ m<-:arr_mats & c<-list_sum | c == max_d ]
	where list_sum = [ sum [ r!!i \\ r<-m & i<-[0..] ] \\ m<-:arr_mats]   
		  max_d = maxList list_sum
...
listToString [] = ""
listToString [x] = toString x
listToString [x:xs] = (toString x) +++ "," +++ listToString xs

help :: (MyList a) -> [a]
help Empty = []
help (Elem x next) = [x] ++ help next 

aux1 :: (MyList a) -> String | toString a
aux1 mylist = "[" +++ listToString (help mylist) +++ "]"

instance toString (MyList a) | toString a
	where toString x = aux1 x
...
instance == (TypeName String)
	where (==) (FirstName a) (FirstName b) = True
		  (==) (MiddleName c) (MiddleName d) = True
		  (==) _ _ = False


getString :: (TypeName String) -> String
getString (FirstName s) = s
getString (MiddleName s) = s

firstAndMiddle :: (Tree (TypeName String)) -> [String]
firstAndMiddle Leaf = []
firstAndMiddle (Node (FirstName s) le ri) = [s] ++ firstAndMiddle le ++ firstAndMiddle ri
firstAndMiddle (Node (MiddleName s) le ri) = [s] ++ firstAndMiddle le ++ firstAndMiddle ri
firstAndMiddle (Node x le ri) = firstAndMiddle le ++ firstAndMiddle ri
...
findWhich [(B c):xs] = 1 + findWhich xs
