...
shiftCipher str 3 = toString[toChar((toInt x)+3) \\ x <- lst ]
	where lst = [a \\ a<-: str]
...
instance == Book
	where (==) a b = a.title == b.title && a.author == b.author && a.num_of_pages==b.num_of_pages
//Start = b1 == b1 // True
//Start = b1 == b2 // False
...
*/
/*
help_lib :: Library Library -> [Library]
help_lib lib1 lib2 = [ a.lib_name +++ b.lib_name \\ a <-lib1 & b<-lib2]

instance + Library
where(+) a b = help_lib a b
*/
//Start = lib1 + lib1
...
helper_lib1 :: Library Library -> Bool
helper_lib1 lib1 lib2 
|l1==l2 && and[a==b\\ a<-: lib1.books & b<-:lib2.books] = True
=False
	where l1=length[a \\ a<-: lib1.books]
		  l2 =length[b\\ b <-: lib2.books]
instance == Library
	where (==) a b = helper_lib1 a b
...
helper_str2 :: String String -> [[String]]
helper_str2 str1 str2 =  flatten[(toString x)+++(toString y) \\ x<-:str1 & y<-:str2]

Start= helper_str2 "hello" "world"

//helper_str :: {String} {String} -> {String}
//helper_str arr1 arr2 = {helper_str2 a b \\ a<-: arr1 & b<-:arr2 }

//instance + {String}
//	where (+) arr1 arr2 = helper_str arr1 arr2
