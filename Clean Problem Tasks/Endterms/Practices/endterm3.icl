...
|n==x = count n (c+1) xs
=count n c xs
//Start = count 1 0 [1,1,1,2,2,2]

occ :: Int [Int] -> Int
occ n list = length [ e \\ e<-list | e == n ]

removeOcc :: {Int} -> {Int}
removeOcc arr = { a \\ a <-:arr  | isOdd (count a 0 lst) }
	where lst = [x \\ x<-: arr]
...
cheapestShop shp =  hd[(a.shopName,a.location) \\ a<-: shp & i <- price_sum | (i==min_price) ]
	where price_sum = [ sum[i.price \\ i <-: a.products] \\ a<-:shp]
		  min_price = minList[ sum[i.price \\ i <-: a.products] \\ a<-:shp]
...
remove_help :: [a] a -> [a] | Eq a
remove_help [] _ = []
remove_help [x:xs] n
| x == n = remove_help xs n
= [x] ++ remove_help xs n

remove :: (Vector a) a -> (Vector a) | Eq a
remove [] n = abort"Element not exist"
remove lst n = remove_help lst n
...
indexOf lst n 
|not (isMember n lst) = abort"Element doesnot exist"
 = hd [ i \\ a<- lst & i<- [0..]| a == n]
...
help_swap :: (Vector a) a a -> (Vector a) | Eq a
help_swap [] n m = []
help_swap [x:xs] n m
| x==n = [m] ++ help_swap xs n m
|x == m = [n] ++ help_swap xs n m
=[x] ++ help_swap xs n m


swap :: (Vector a) a a-> (Vector a) | Eq a
swap lst n m
|not (isMember n lst) || not (isMember m lst) = abort"Element does not exist"
= help_swap lst n m
...
flexTreeToList (UnaryNode x next) =[x] ++ (flexTreeToList next) 
