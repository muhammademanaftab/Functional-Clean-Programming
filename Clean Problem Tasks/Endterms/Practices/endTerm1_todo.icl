...
instance toInt Person
	where toInt a = size (a.firstName) + a.age+a.height
...
instance isEven Person
	where isEven a = isEven(a.age+a.height)
...
count str c = length [1 \\ a <- ls | a==c ]
	where ls = [a \\ a<-:str]

continentsPrimeI :: [Continent] -> [String]
continentsPrimeI ls_c = flatten[[ a.contName \\ x <-: a.countries | isprime(count x.capital 'i')] \\ a <- ls_c ]
...
shouldWePlay pl_ls gp = length [ a \\ a<- pl_ls | a.skillLevel >=gp.skillLevel] >=3
...
count_help :: [Int] Int -> Int
count_help ls n = length [1 \\ a<-ls | a==n] 


f7 :: {Int} -> Bool
f7 arr = and [(count_help ls a)>=2 \\ a <- ls ]
	where ls = [a \\ a<-:arr] 
